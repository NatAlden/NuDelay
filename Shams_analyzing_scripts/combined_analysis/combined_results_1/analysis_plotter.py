...
patt_Highlow_array_scan = "patt_Highlow_array_scan_modified_delays.json"
patt_phased_array_scan = "patt_phased_array_scan.json"
...
plt.plot(patt_phased_angle_list, patt_phased_mid_points, 'v', label='PATT Phased Array scan', color='orange')
plt.plot(patt_Highlow_angle_list, patt_Highlow_mid_points, 'x', label='PATT Highlow scan modified', color='green')
...
plt.title("Simulated vs PATT Measured SNR at 50% Efficiency")
plt.legend( loc='upper left', fontsize=8)
plt.grid(True)
plt.ylim(1.8, 5.0)
plt.tight_layout()
plt.savefig("Simulated_vs_PATT_Measured_SNR_at_50_Efficiency_modified_delays.png", dpi=300)
plt.show()
