...
SNR_slope =  0.2987923898864614  # SNR slope from the fit in get_SNR_from_p2p.py
...
# Load the data
with open("efficiency_curve_phased_at_4,5deg.json", "r") as f:
    data = json.load(f)
...
ax.set_title("Trigger Efficiency Curve at 4.5 Degrees Phased Trigger")
ax.grid(True)
ax.set_xlim(1, 8)
ax.set_ylim(0, 1.1)
plt.tight_layout()
plt.savefig("phased_trigger_efficiency_curve_4.5deg.png", dpi=300)