...
k = 0.026
SWEEP_NS = 3
DURATION = 2  # seconds total per channel sweep
STEPS = 100
STEP_DELAY = DURATION / (2 * STEPS)

# === Channel Maps ===
# attenuation index → label
channel_map_atten = {0: 'C4', 2: 'C2', 3: 'C1'}

# time delay index → label
channel_map_delay = {1: 'C4', 2: 'C1', 3: 'C2'}

origins = {'C1': 0.6, 'C2': 2.03, 'C4': 0.1}
i2c_addresses = {'C4': 0x38, 'C2': 0x3c, 'C1': 0x3e}

# === I2C Setup for Attenuators ===
bus = smbus.SMBus(2)
output_reg = 0x01
config_reg = 0x03
...
def animate_channel(channel_label):
    ...
    # Sweep forward (delay ↑, attenuation ↑)
    for i in range(STEPS + 1):
        delay_val = origin + (SWEEP_NS * i / STEPS)
        percent = max(0, 100 - (i * (100 / STEPS)))
        atten_val = percent_to_attenuation(percent)
        set_delay(channel_label, delay_val)
        setAttenuation(addr, atten_val)
        time.sleep(STEP_DELAY)

    # Sweep back (delay ↓, attenuation ↓)
    for i in range(STEPS + 1):
        delay_val = origin + (SWEEP_NS - SWEEP_NS * i / STEPS)
